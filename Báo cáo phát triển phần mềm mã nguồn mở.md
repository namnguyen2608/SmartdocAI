...
[**5.2.** **Tính Năng Chính và Hướng Dẫn Sử Dụng**](#52-các-tính-năng-chính)
...
* **Chương I** trình bày bối cảnh, mục tiêu dự án và tổng quan các công trình liên quan.
...
* **Chương V** mô tả giao diện người dùng, danh sách tính năng và hướng dẫn sử dụng.
...
Dự án nghiên cứu và phát triển hệ thống hỏi đáp tài liệu thông minh SmartDoc AI dựa trên kiến trúc Retrieval-Augmented Generation (RAG), hoạt động hoàn toàn ngoại tuyến trên phần cứng cá nhân. Hệ thống hướng đến ba mục tiêu cụ thể:

* **Xây dựng đường ống RAG đa kỹ thuật:** Tích hợp Hybrid Search (BM25 + FAISS), Cross-Encoder Re-Ranking, Self-RAG và kiến trúc Co-RAG đa agent để tối ưu chất lượng truy xuất và giảm hallucination.
* **Hỗ trợ đa định dạng và đa ngôn ngữ:** Xử lý tài liệu PDF và DOCX; tự động phát hiện và phản hồi đúng ngôn ngữ, đặc biệt tối ưu cho tiếng Việt.
* **Triển khai LLM cục bộ bảo đảm bảo mật:** Vận hành Qwen2.5:7b qua Ollama trực tiếp trên máy tính cá nhân, loại bỏ hoàn toàn rủi ro rò rỉ dữ liệu qua API bên ngoài.
...
Để hỗ trợ hội thoại liên tục (multi-turn conversation), hệ thống triển khai cơ chế viết lại câu hỏi (Question Reformulation): khi người dùng đặt câu hỏi tiếp theo, LLM phân tích lịch sử hội thoại và tự động viết lại thành một câu hỏi độc lập trước khi thực hiện truy xuất. Chi tiết thiết kế và prompt template được trình bày trong §3.4 và §6.3.

## **2.6. Lọc Metadata Theo Tài Liệu Nguồn**  
Hệ thống hỗ trợ lọc kết quả truy xuất theo metadata `source` (tên tệp tài liệu). Người dùng chọn file từ dropdown trong sidebar để giới hạn phạm vi tìm kiếm, loại bỏ kết quả từ các tệp không liên quan. Phân tích thiết kế chi tiết tại §6.2.2.
...
Sau bước retrieval (bi-encoder), hệ thống thực hiện re-ranking sử dụng Cross-Encoder để cải thiện độ chính xác. Khác với bi-encoder (mã hóa câu hỏi và tài liệu thành hai vector độc lập), Cross-Encoder nhận cả câu hỏi và tài liệu làm đầu vào cùng lúc (`[CLS] question [SEP] document [SEP]`), cho phép tính toán tương tác từng từ giữa hai văn bản — cho điểm relevance chính xác hơn với chi phí O(n) inference calls. SmartDoc AI sử dụng `cross-encoder/ms-marco-MiniLM-L-6-v2` fine-tune trên MS MARCO. Phân tích thiết kế chi tiết tại §6.2.3.

## **2.9. Self-RAG — Tự Đánh Giá và Cải Thiện**

Self-RAG (Asai et al., 2023) [4] là kỹ thuật giúp LLM tự đánh giá chất lượng quá trình retrieval và generation qua ba thành phần: (1) **Query Rewriting** — sinh 3 phiên bản câu hỏi từ góc độ khác nhau để mở rộng không gian tìm kiếm; (2) **Relevance Grading** — loại tài liệu không liên quan trước khi đưa vào prompt; (3) **Answer Grading** — kiểm tra hallucination, trả về JSON với điểm confidence $[0.0, 1.0]$. Phân tích thiết kế chi tiết tại §6.4.

## **2.10. Co-RAG — Truy Xuất Hợp Tác Đa Agent**

Co-RAG (Collaborative RAG) mở rộng RAG truyền thống bằng cách triển khai **nhiều retriever agent song song**, mỗi agent khai thác một chiều không gian tìm kiếm khác nhau. Một thành phần **Consensus Merger** tổng hợp kết quả qua cơ chế bầu chọn [3].
...
trong đó $\bar{s}$ là điểm trung bình từ các agent đồng thuận và $v$ là số agent trả về tài liệu đó. Ba chiến lược hợp nhất: `voting` (mặc định, giữ tài liệu có ≥ $\text{CO\_RAG\_MIN\_VOTES}=2$ agent), `union` (tối đa hoá độ bao phủ) và `intersection` (độ chính xác cao nhất). Phân tích thiết kế chi tiết tại §6.5.
...
│  • Giao diện web tương tác (light theme)                 │
...
| **Bo mạch chủ** | ASUSTeK TUF GAMING B760M-PLUS WIFI D4 |
| **CPU** | Intel Core i5-12400F (Alder Lake) — 6 nhân / 12 luồng, 2.500 MHz, hỗ trợ AVX2 |
| **RAM** | 24 GB DDR4 |
| **GPU** | Không sử dụng — LLM inference và embedding chạy hoàn toàn trên CPU (Ollama) |

#### **Môi Trường Phần Mềm**

| Thư viện / Công cụ | Phiên bản | Vai trò |
|---|---|---|
| Python | >=3.8 | Nền tảng thực thi |
| Ollama | latest | Vận hành LLM cục bộ (Qwen2.5:7b) |
| LangChain + Community + Ollama + HuggingFace | >=0.2.0 | Điều phối pipeline RAG |
| faiss-cpu | >=1.7.4 | Vector store và similarity search |
| rank-bm25 | >=0.2.2 | Tìm kiếm từ khóa BM25 |
| pdfplumber | >=0.11.0 | Trích xuất văn bản từ PDF (PyPDF2 dự phòng trong requirements) |
| python-docx | >=1.1.0 | Trích xuất văn bản và bảng từ DOCX |
| sentence-transformers + torch | >=2.2.0 / >=2.0.0 | Embedding đa ngôn ngữ cục bộ |
| Streamlit | >=1.30.0 | Giao diện người dùng web |
...
SmartDoc AI triển khai giao diện web bằng Streamlit với **light theme** toàn cục, tổ chức thành hai vùng chính:
...
Mở trình duyệt tại `http://localhost:8501`. Upload file PDF/DOCX → đợi thông báo "Đã xử lý X chunks" → nhập câu hỏi vào ô chat → xem câu trả lời kèm nguồn trích dẫn. SmartDoc AI nhớ lịch sử hội thoại trong session — có thể hỏi liên tiếp bằng đại từ tham chiếu mà không cần nhắc lại chủ đề. Để tinh chỉnh hành vi hệ thống, chỉnh các tham số trong `config.py` (xem Bảng cấu hình §3.6.3).
...
|---|---|---|---|
| **Cơ chế** | Cosine similarity (vector) | Term frequency (TF-IDF) | RRF tổng hợp thứ hạng |
| **Điểm mạnh** | Hiểu ngữ nghĩa, paraphrase | Khớp từ khóa chính xác | Kết hợp cả hai |
| **Điểm yếu** | Miss từ chuyên ngành cụ thể | Không hiểu đồng nghĩa | Latency cao hơn |
| **Trọng số** | 0.6 | 0.4 | — |
...
Cross-encoder xử lý cặp (câu hỏi, document) đồng thời qua transformer với full cross-attention, cho độ chính xác cao hơn đáng kể. Tuy nhiên, chi phí tính toán là O(n) forward pass — không thể dùng để tìm kiếm trực tiếp trên toàn bộ index. Giải pháp là kiến trúc **2 giai đoạn**:

1. **Bi-encoder recall** (FAISS): fetch nhanh top-50 candidates bằng cosine similarity.
2. **Cross-encoder re-rank**: tính cross-attention chính xác cho top-50, giữ lại top-3 đưa vào context LLM.
...
Streamlit re-run toàn bộ script Python sau mỗi tương tác người dùng — đây là đặc điểm kiến trúc, không phải lỗi. Ba quyết định thiết kế chính để duy trì trạng thái:

* **Lưu trữ bằng `st.session_state`**: cơ chế persistent storage duy nhất của Streamlit, tồn tại qua các lần re-run trong cùng phiên trình duyệt. Mọi trạng thái (lịch sử chat, Vector Store) đều được gắn vào đây.
* **Định dạng lịch sử hội thoại**: lưu dưới dạng list các dict `{"role": "user"/"assistant", "content": "..."}` — định dạng chuẩn tương thích trực tiếp với pipeline reformulation trong `rag_chain.py`.
* **Hai nút xóa độc lập**: "Xóa lịch sử hội thoại" reset danh sách message nhưng giữ nguyên Vector Store (phù hợp khi đổi chủ đề, giữ tài liệu); "Xóa Vector Store" dùng `shutil.rmtree()` để xóa toàn bộ thư mục index (FAISS lưu đồng thời `index.faiss` + `index.pkl` — xóa riêng lẻ để lại trạng thái không nhất quán).
...
RAG chuẩn không có cơ chế kiểm soát chất lượng nội sinh: nếu retriever trả về chunks không liên quan, LLM vẫn cố tổng hợp câu trả lời và có thể hallucinate. Self-RAG triển khai ba tầng kiểm soát độc lập, mỗi tầng giảm một loại lỗi cụ thể:

| Tầng | Thời điểm | Cơ chế | Mục tiêu |
|---|---|---|---|
| **1 — Query Expansion** | Trước retrieval | LLM sinh 3 phiên bản câu hỏi; retrieve song song và hợp nhất | Tăng recall, giảm miss rate |
| **2 — Relevance Grading** | Sau retrieval, trước generation | LLM chấm "CÓ/KHÔNG" từng document; loại document không liên quan | Giảm nhiễu, giảm hallucination |
| **3 — Answer Grading** | Sau generation | LLM đánh giá 4 chiều: `score`, `is_grounded`, `has_hallucination`, `feedback` | Phát hiện hallucination, cảnh báo người dùng |
...
Về mặt kỹ thuật, hệ thống tích hợp Qwen2.5:7b (qua Ollama), LangChain và Streamlit thành một pipeline hoàn chỉnh. Bốn kỹ thuật retrieval nâng cao được triển khai:
