...
[**6.11.** **Q11 — Co-RAG (Truy Xuất Hợp Tác Đa Agent)**](#611-q11--co-rag-truy-xuất-hợp-tác-đa-agent)
...
* **Chương VI** trả lời toàn diện 11 yêu cầu phát triển (Q1–Q11) theo rubric đánh giá, mô tả giải pháp thiết kế và luồng xử lý của từng tính năng.
...
# **CHƯƠNG VI: YÊU CẦU PHÁT TRIỂN (Q1–Q11)**

Chương này trình bày chi tiết 11 yêu cầu kỹ thuật (Q1-Q11) của đề tài, giải thích thiết kế giải pháp và bằng chứng kiểm thử.
...
**Thiết kế giải pháp** (`modules/document_processor.py`):

Mô-đun định nghĩa tập `SUPPORTED_EXTENSIONS = {".pdf", ".docx"}` và hai hàm chuyên biệt. Hàm `extract_text_from_pdf()` dùng thư viện `pdfplumber` để mở file PDF và duyệt từng trang, trích xuất văn bản thô; mỗi trang thành công được bọc trong đối tượng `Document` của LangChain với metadata đầy đủ: `source` (tên file), `page`, `total_pages`, `file_type`, `upload_date`. Hàm `extract_text_from_docx()` dùng thư viện `python-docx` để đọc các paragraph, sau đó gộp nhóm mỗi 20 đoạn thành một "trang ảo" — giả lập khái niệm trang cho định dạng không có phân trang thực. Cả hai hàm đều bỏ qua các trang/đoạn trống để tránh tạo chunk vô nghĩa.

*Source: modules/document_processor.py*

**Kết quả kiểm thử:** 3 test cases passed — `test_extract_pdf`, `test_extract_docx`, `test_unsupported_extension_raises`.
...
**Thiết kế giải pháp** (`app.py`):

Lịch sử hội thoại được lưu trong `st.session_state.chat_history` — một danh sách xen kẽ các đối tượng `HumanMessage` và `AIMessage` từ LangChain. Danh sách được khởi tạo rỗng khi ứng dụng khởi động và được append sau mỗi lượt hội thoại thành công. Toàn bộ danh sách được truyền vào hàm `ask_question()` dưới dạng tham số `chat_history`, cho phép pipeline RAG tận dụng ngữ cảnh các lượt trước khi xử lý câu hỏi mới. Giao diện chat hiển thị lịch sử theo dạng bubble phân biệt màu sắc người dùng/AI bằng `st.chat_message()`.
...
**Thiết kế giải pháp** (`app.py` — sidebar):

Hai nút độc lập được đặt trên sidebar. Nút **"Xóa lịch sử hội thoại"** chỉ reset `st.session_state.chat_history = []` rồi gọi `st.rerun()` — giữ nguyên tài liệu đã index trong FAISS. Nút **"Xóa Vector Store"** thực hiện reset toàn diện: đặt `st.session_state.vector_store = None`, xóa lịch sử, và dùng `shutil.rmtree()` để xóa toàn bộ thư mục index FAISS trên đĩa (`config.FAISS_INDEX_PATH`). Sự tách biệt này cho phép người dùng linh hoạt bắt đầu cuộc hội thoại mới mà không cần nạp lại tài liệu.
...
Hàm `split_documents()` sử dụng `RecursiveCharacterTextSplitter` của LangChain với chiến lược tách phân cấp: ưu tiên tách tại `\n\n` (ranh giới đoạn văn), sau đó `\n` (ranh giới dòng), rồi dấu câu (`.`, `!`, `?`), cuối cùng mới cắt theo ký tự. Điều này đảm bảo chunks không bị cắt giữa câu hoàn chỉnh.

Tham số được chọn qua thực nghiệm:

| Tham số | Giá trị | Lý do |
|---|---|---|
| `CHUNK_SIZE` | 1500 ký tự | ~300–400 tokens — đủ ngữ cảnh, phù hợp context window |
| `CHUNK_OVERLAP` | 200 ký tự | ~13% overlap — bảo toàn câu ở ranh giới giữa hai chunks |

*Source: modules/document_processor.py, config.py*
...
**Thiết kế giải pháp** (`modules/rag_chain.py`):

Hàm `ask_question()` trả về dictionary gồm `answer`, `sources` và `language`. Danh sách `sources` được xây dựng bằng cách duyệt qua các `retrieved_docs` sau bước retrieval: với mỗi document, trích `source` (tên file) và `page` từ metadata, dùng set để dedup — tránh liệt kê cùng một trang nhiều lần nếu nhiều chunk đến từ đó. Giao diện hiển thị danh sách nguồn dưới mỗi câu trả lời với icon 📄, tên file và số trang.

*Source: modules/rag_chain.py*

**Kết quả kiểm thử:** 4 test cases passed — `test_sources_returned_in_response`, `test_source_contains_file_and_page`, `test_sources_deduplicated`, `test_format_context_includes_source_info`.
...
**Thiết kế giải pháp** (`modules/rag_chain.py` — Question Reformulation):

Khi `chat_history` không rỗng, hàm `ask_question()` không truyền câu hỏi gốc trực tiếp vào retriever. Thay vào đó, một bước **reformulation** được thực thi trước: LLM nhận lịch sử hội thoại đã format và câu hỏi tiếp theo, viết lại thành câu hỏi standalone hoàn chỉnh, giải quyết mọi tham chiếu đại từ. Câu hỏi đã viết lại sau đó được dùng cho retrieval và generation. Khi không có lịch sử (câu hỏi đầu tiên), bước reformulation bị bỏ qua để tiết kiệm một lần gọi LLM.

*Source: modules/rag_chain.py*

**Kết quả kiểm thử:** 4 test cases passed — `test_chat_history_included_in_prompt`, `test_reformulate_template_has_required_vars`, `test_empty_history_no_reformulation`, `test_ask_question_accepts_chat_history_param`.
...
**Thiết kế giải pháp** (`modules/vector_store.py`):

Hàm `create_bm25_retriever()` khởi tạo `BM25Retriever` từ danh sách documents với `k = HYBRID_TOP_K`. Hàm `create_ensemble_retriever()` kết hợp FAISS retriever (dùng MMR, `k=HYBRID_TOP_K`) và BM25 retriever thông qua `EnsembleRetriever` của LangChain, áp dụng **Reciprocal Rank Fusion** để hợp nhất danh sách kết quả từ hai nguồn. Trọng số mặc định: FAISS 0.6, BM25 0.4 — phản ánh ưu tiên ngữ nghĩa cao hơn khớp từ khóa.

*Source: modules/vector_store.py, config.py*

**Kết quả kiểm thử:** 5 test cases passed — `test_ensemble_retriever_created`, `test_retriever_weights_correct`, `test_bm25_retriever_created`, `test_hybrid_returns_documents`, `test_hybrid_search_end_to_end` (integration).
...
Hàm `similarity_search()` nhận tham số tùy chọn `file_filter`. Khi được cung cấp, hàm retrieve `k×3` documents trước (để đảm bảo đủ candidates), sau đó lọc chỉ giữ các doc có `metadata["source"] == file_filter`, cuối cùng trả về top-k từ tập đã lọc. Hàm tiện ích `filter_by_source()` hỗ trợ lọc trên danh sách documents tùy ý. Lọc là case-sensitive — tên file phải khớp chính xác với metadata đã lưu khi embedding.

*Source: modules/vector_store.py*

**Kết quả kiểm thử:** 9 test cases passed — bao gồm kiểm tra lọc đúng, lọc sai nguồn, lọc file không tồn tại trả về rỗng, và 2 integration tests.
...
**Thiết kế giải pháp** (`modules/reranker.py`):

Model `cross-encoder/ms-marco-MiniLM-L-6-v2` được nạp theo pattern Singleton (hàm `get_cross_encoder()`) để tránh tải lại model nhiều lần. Hàm `rerank_with_cross_encoder()` nhận danh sách `(Document, bi_encoder_score)` từ bước retrieval: tạo các cặp `(query, document_content)`, cho qua Cross-Encoder để lấy raw scores, chuẩn hóa về [0, 1] bằng hàm sigmoid $\sigma(x) = \frac{1}{1+e^{-x}}$, rồi sắp xếp giảm dần theo cross-encoder score và cắt lấy top-k. Cơ chế fallback tự động về bi-encoder scores nếu cross-encoder gặp lỗi, đảm bảo hệ thống không bị gián đoạn.

*Source: modules/reranker.py*

**Kết quả kiểm thử:** 5 test cases passed — `test_rerank_returns_sorted_list`, `test_rerank_top_k_respected`, `test_scores_normalized_sigmoid`, `test_fallback_when_model_unavailable`, `test_cross_encoder_rerank_real` (integration).
...
**Thiết kế giải pháp** (`modules/self_rag.py`):

Mô-đun triển khai ba hàm độc lập, mỗi hàm gọi LLM với prompt chuyên biệt:

- **`rewrite_query()`:** Dùng LLM sinh 3 phiên bản câu hỏi khác nhau (đổi từ ngữ, nhấn mạnh khía cạnh khác) để mở rộng không gian tìm kiếm. Kết quả là danh sách 3 câu hỏi dùng để retrieve song song.
- **`grade_document_relevance()`:** LLM đánh giá từng document có liên quan đến câu hỏi không, trả về "CÓ" hoặc "KHÔNG". Chỉ các document được đánh giá "CÓ" mới được đưa vào context cho bước generation.
- **`grade_answer()`:** Sau khi sinh câu trả lời, LLM đánh giá toàn diện và trả về JSON với 4 trường: `score` (0.0–1.0), `is_grounded` (có căn cứ từ context), `has_hallucination` (phát hiện thông tin bịa), `feedback` (giải thích). Kết quả này được hiển thị trên UI để người dùng tham khảo.

*Source: modules/self_rag.py*

**Kết quả kiểm thử:** 7 test cases passed — bao gồm 3 unit tests và 3 integration tests với Ollama thực.

## **6.11. Q11 — Co-RAG (Truy Xuất Hợp Tác Đa Agent)**

**Yêu cầu:** Hệ thống hỗ trợ chế độ truy xuất hợp tác đa agent (Co-RAG), trong đó nhiều agent khai thác các chiều không gian tìm kiếm khác nhau và kết quả được hợp nhất qua cơ chế bầu chọn, nhằm cải thiện recall cho câu hỏi đa bước.

**Thiết kế giải pháp** (`modules/co_rag.py`):

Co-RAG triển khai ba agent retrieval song song, mỗi agent với chiến lược khác nhau, sau đó Consensus Merger tổng hợp kết quả:
...
- **Agent 1 (Semantic):** `semantic_retriever_agent()` — FAISS `similarity_search_with_relevance_scores()` trả về top-K docs theo ngữ nghĩa.
- **Agent 2 (Keyword):** `keyword_retriever_agent()` — BM25Retriever tìm kiếm theo từ khóa chính xác, điểm xấp xỉ theo thứ hạng.
- **Agent 3 (Conceptual):** `conceptual_decomposer_agent()` — LLM phân rã câu hỏi phức tạp thành ≤3 sub-questions, FAISS retrieve riêng từng câu rồi dedup.
- **Consensus Merger:** `consensus_merger()` — Tài liệu được nhiều agent đồng thuận nhận vote boost: $\text{score}_{\text{merged}} = \bar{s} \times (1 + (v-1) \times 0.15)$. Chiến lược mặc định `voting` chỉ giữ docs có ≥ `CO_RAG_MIN_VOTES = 2` agent đồng ý.

**Tích hợp UI:** Co-RAG và Self-RAG là mutually exclusive — người dùng bật một trong hai từ sidebar. Kết quả trả về kèm metadata: số docs trước/sau merge, số agent, sub-questions được sinh ra.

*Source: modules/co_rag.py*
