...
**Xác minh kiểm thử:** Trích xuất text và metadata đúng từ PDF (theo từng trang) và DOCX (nhóm 20 đoạn thành trang ảo); định dạng không hỗ trợ bị từ chối bằng `ValueError`.
...
**Xác minh kiểm thử:** Response luôn có trường `sources`; mỗi source chứa đủ `file` và `page`; dedup hoạt động đúng khi nhiều chunk đến từ cùng trang.
...
**Xác minh kiểm thử:** Lịch sử được đưa vào prompt reformulation; template chứa đúng hai biến `{chat_history}` và `{question}`; câu hỏi đầu tiên (không có history) bỏ qua bước reformulation để tiết kiệm một lần gọi LLM.
...
**Xác minh kiểm thử:** EnsembleRetriever tạo thành công; trọng số FAISS=0.6/BM25=0.4 đúng theo config; BM25Retriever tạo độc lập; hybrid search end-to-end trả về danh sách Document hợp lệ.
...
**Xác minh kiểm thử:** Lọc đúng file target; docs từ file khác bị loại; file không tồn tại trả về list rỗng; metadata nguyên vẹn sau lọc; phân biệt hoa thường; isolation hoạt động đúng khi nhiều file cùng tồn tại trong Vector Store.
...
**Xác minh kiểm thử:** Danh sách kết quả sắp xếp đúng theo cross-encoder score; top-k được tuân thủ; score sigmoid ∈ [0, 1]; fallback kích hoạt khi model không khả dụng; integration test với model thực xác nhận thứ tự reranking đúng.
...
**Xác minh kiểm thử:** Ba hàm trả về đúng định dạng output — `rewrite_query()` sinh list ≥1 câu; `grade_document_relevance()` chỉ trả "CÓ" hoặc "KHÔNG"; `grade_answer()` trả JSON với đủ 4 trường và score ∈ [0.0, 1.0]; integration test với Ollama xác nhận response được parse đúng.
