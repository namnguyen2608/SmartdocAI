...
[**4.4.** **Trực Quan Hóa Dữ Liệu**](#44-trực-quan-hóa-dữ-liệu)
...
[**CHƯƠNG VI.** **Yêu Cầu Phát Triển (Q1–Q10)**](#chương-vi-yêu-cầu-phát-triển-q1-q10)

[**6.1.** **Q1 — Xử Lý Tài Liệu Đa Định Dạng (PDF & DOCX)**](#61-q1--xử-lý-tài-liệu-đa-định-dạng-pdf--docx)

[**6.2.** **Q2 — Lưu Trữ Lịch Sử Hội Thoại**](#62-q2--lưu-trữ-lịch-sử-hội-thoại)

[**6.3.** **Q3 — Xóa Lịch Sử và Vector Store**](#63-q3--xóa-lịch-sử-và-vector-store)

[**6.4.** **Q4 — Chunking Tài Liệu**](#64-q4--chunking-tài-liệu-phân-đoạn-văn-bản)

[**6.5.** **Q5 — Citation Tracking (Truy Dấu Nguồn)**](#65-q5--citation-tracking-truy-dấu-nguồn)

[**6.6.** **Q6 — Conversational RAG (Hội Thoại Đa Lượt)**](#66-q6--conversational-rag-hội-thoại-đa-lượt)

[**6.7.** **Q7 — Hybrid Search (FAISS + BM25)**](#67-q7--hybrid-search-faiss--bm25)

[**6.8.** **Q8 — Metadata Filtering (Lọc Theo Tài Liệu Nguồn)**](#68-q8--metadata-filtering-lọc-theo-tài-liệu-nguồn)

[**6.9.** **Q9 — Cross-Encoder Re-Ranking**](#69-q9--cross-encoder-re-ranking)

[**6.10.** **Q10 — Self-RAG (Tự Đánh Giá và Cải Thiện)**](#610-q10--self-rag-tự-đánh-giá-và-cải-thiện)
...
Mục tiêu cốt lõi của dự án là chứng minh rằng một hệ thống AI hỏi đáp tài liệu có khả năng xử lý đa ngôn ngữ (đặc biệt tiếng Việt), hỗ trợ nhiều định dạng tệp (PDF, DOCX), và tích hợp các kỹ thuật retrieval nâng cao (Hybrid Search, Re-Ranking, Self-RAG) hoàn toàn có thể được triển khai trên phần cứng cá nhân mà không cần phụ thuộc vào các dịch vụ đám mây hay tiềm ẩn rủi ro bảo mật dữ liệu.
...
* **Chương II** xây dựng nền tảng lý thuyết đầy đủ: RAG, Embedding, FAISS, LLM, LangChain, Hybrid Search, Cross-Encoder Re-Ranking và Self-RAG.
...
* **Chương VI** trả lời toàn diện 10 yêu cầu phát triển (Q1–Q10) theo rubric đánh giá, với code minh họa trực tiếp từ source code.
...
Nghiên cứu của Lewis và các cộng sự (2020) đã đặt nền móng cho RAG bằng cách chứng minh rằng việc kết hợp một bộ truy xuất dữ liệu ngoại vi với một mô hình sinh văn bản có thể cải thiện đáng kể độ chính xác trong các tác vụ cần nhiều tri thức. Tiếp nối hướng đi này, các framework như LangChain và LlamaIndex đã ra đời, cung cấp các công cụ chuẩn hóa để xây dựng các đường ống (pipelines) xử lý dữ liệu từ nhiều định dạng như PDF, Word, hay Markdown một cách hiệu quả.

Bên cạnh đó, xu hướng Local LLM (triển khai mô hình ngôn ngữ cục bộ) đang ngày càng phát triển nhờ các nền tảng như Ollama. Điều này cho phép người dùng vận hành các mô hình mạnh mẽ như Qwen2.5 ngay trên phần cứng cá nhân mà không cần kết nối internet, đảm bảo tính riêng tư tuyệt đối cho dữ liệu doanh nghiệp và cá nhân.

Đồ án SmartDoc AI được xây dựng dựa trên sự kế thừa các thành tựu này, tập trung tối ưu hóa khả năng truy xuất cho ngôn ngữ tiếng Việt – một lĩnh vực đòi hỏi sự tinh chỉnh đặc biệt về mô hình Embedding và kỹ thuật phân mảnh văn bản.

# **CHƯƠNG II: CƠ SỞ LÝ THUYẾT** {#chương-ii-cơ-sở-lý-thuyết}

## **2.1. Kiến Trúc RAG (Retrieval-Augmented Generation)** {#21-kiến-trúc-rag-retrieval-augmented-generation}  
Kiến trúc RAG là một quy trình hai giai đoạn nhằm nâng cao chất lượng phản hồi của AI bằng cách tích hợp thông tin từ một nguồn dữ liệu đáng tin cậy bên ngoài.
...
### **2.2.2. FAISS (Facebook AI Similarity Search)** {#222-faiss-facebook-ai-similarity-search}  
...
* **EnsembleRetriever:** LangChain cung cấp `EnsembleRetriever` (thông qua gói `langchain_classic`) để kết hợp kết quả từ nhiều retriever bằng thuật toán **Reciprocal Rank Fusion (RRF)**. Mỗi retriever được gán một trọng số; tài liệu xuất hiện cao ở cả hai danh sách sẽ được xếp hạng ưu tiên trong kết quả cuối. Trong SmartDoc AI, FAISS chiếm trọng số 0.6 và BM25 chiếm 0.4.
...
│  • Hybrid Search / Re-Ranking / Self-RAG                 │
...
## **4.1. Môi Trường và Thiết Lập Thí Nghiệm** {#41-môi-trường-và-thiết-lập-thí-nghiệm}
### **4.1.1. Thiết Lập Môi Trường và Dữ Liệu Thực Nghiệm** {#411-thiết-lập-môi-trường-và-dữ-liệu-thực-nghiệm}

#### **Môi Trường Phần Cứng**  
Việc vận hành mô hình ngôn ngữ lớn (LLM) nội bộ đòi hỏi tài nguyên tính toán nhất định. Hệ thống thử nghiệm được thiết lập với cấu hình phần cứng như sau:

* Vi xử lý (CPU): Tối thiểu bộ vi xử lý đa nhân kiến trúc x86\_64 hoặc ARM64 (ví dụ: Intel Core i5 thế hệ thứ 8 trở lên, hoặc AMD Ryzen 5 3000 Series trở lên). CPU cần hỗ trợ các tập lệnh xử lý vector (như AVX2) để tăng tốc độ truy xuất của FAISS.  
* Bộ nhớ trong (RAM): Yêu cầu tối thiểu 8GB RAM để có thể tải các trọng số (weights) đã được lượng tử hóa (quantized) của mô hình 7B vào bộ nhớ. Tuy nhiên, mức dung lượng 16GB RAM được khuyến nghị làm tiêu chuẩn tối thiểu thực tế để hệ thống có thể vừa xử lý tài liệu, vừa chạy UI và hệ điều hành mà không gặp hiện tượng tràn bộ nhớ (Out of Memory).  
* Bộ xử lý đồ họa (GPU \- Nếu có): Hệ thống được thiết kế để có thể hoạt động hoàn toàn dựa trên CPU thông qua cơ chế tối ưu của Ollama. Dù vậy, để đạt được thời gian sinh câu trả lời ở mức chấp nhận được (dưới 10 giây), hệ thống yêu cầu tối thiểu một GPU rời có dung lượng VRAM từ 4GB đến 6GB (ví dụ: NVIDIA GTX 1650, RTX 3050 hoặc tương đương).  
* Lưu trữ: Tối thiểu 15GB dung lượng trống trên ổ cứng SSD. Việc sử dụng ổ cứng thể rắn (SSD) là bắt buộc để đảm bảo băng thông đọc/ghi khi hệ thống nạp mô hình LLM có dung lượng lớn vào RAM và khi thực hiện tìm kiếm trên cơ sở dữ liệu vector.
...
### **4.1.2. Dữ Liệu Thực Nghiệm** {#412-dữ-liệu-thực-nghiệm}  
...
**Câu truy vấn (Queries):** Tập hợp 5 câu lệnh mẫu phức tạp liên quan đến AI và RAG. Hệ thống đo thời gian trung bình (milliseconds) mà mỗi thuật toán cần để trả về Top 5 tài liệu liên quan nhất cho mỗi kích thước dữ liệu (trung bình qua 5 lần đo mỗi query, tổng 25 lượt đo mỗi mức chunk).
## **4.2. Kết Quả Thí Nghiệm** {#42-kết-quả-thí-nghiệm}  
...
| :---: | :---: | :---: | :---: |
| **1.000** | 41.18 | 3.15 | 42.54 |
| **3.000** | 39.61 | 5.39 | 49.69 |
| **5.000** | 39.87 | 10.77 | 48.77 |
| **10.000** | 50.11 | 35.77 | 74.65 |

*Nhận xét Bảng I:*

* **BM25:** Độ trễ tăng tuyến tính (3.15ms → 35.77ms) khi số chunk tăng, do phải duyệt toàn bộ chỉ mục từ vựng (sparse index).
* **FAISS:** Độ trễ ổn định quanh 40–50ms bất kể kích thước dữ liệu — phần tìm kiếm vector (1–8ms) bị chi phối bởi bước encode câu hỏi (~40ms, hằng số cố định).
* **Hybrid Search:** Độ trễ ~43–75ms do tổng hợp kết quả từ cả hai retriever và thực hiện Reciprocal Rank Fusion.

## **4.3. Kiểm Thử Phần Mềm (Testing)** {#43-kiểm-thử-phần-mềm-testing}
...
### **4.3.1. Cấu Trúc Thư Mục Kiểm Thử** {#431-cấu-trúc-thư-mục-kiểm-thử}
...
### **4.3.2. Phân Loại Test Cases** {#432-phân-loại-test-cases}

**Bảng II.** Danh sách test cases theo module và loại kiểm thử (**Tổng: 141 tests passed**)

| Module | Loại | Tên Test Case | Mô tả |
|---|---|---|---|
| `language_detector` | Unit | `test_detect_vi_with_accents` | Câu tiếng Việt có dấu → 'vi' |
| `language_detector` | Unit | `test_detect_en_sentences` | Câu tiếng Anh → 'en' |
| `language_detector` | Unit | `test_empty_string_returns_en` | Chuỗi rỗng → 'en' (không crash) |
| `language_detector` | Unit | `test_returns_str_type` | Kiểu trả về luôn là `str` |
| `language_detector` | Unit | `test_get_prompt_contains_context` | Template chứa `{context}` và `{question}` |
| `document_processor` | Unit | `test_empty_input_returns_empty` | Input rỗng → list rỗng |
| `document_processor` | Unit | `test_metadata_preserved` | Metadata nguồn được giữ nguyên |
| `document_processor` | Unit | `test_chunk_size_respected` | Chunk không vượt quá `chunk_size` |
| `document_processor` | Unit | `test_unsupported_extension_raises` | `.xyz` → `ValueError` |
| `document_processor` | Integration | `test_extract_pdf` | PDF thật → danh sách Document |
| `document_processor` | Integration | `test_extract_docx` | DOCX thật → danh sách Document |
| `rag_chain` (Q5) | Unit | `test_sources_returned_in_response` | `ask_question()` trả về `sources` list |
| `rag_chain` (Q5) | Unit | `test_source_contains_file_and_page` | Mỗi source có key `file` và `page` |
| `rag_chain` (Q5) | Unit | `test_sources_deduplicated` | Không trùng lặp nguồn trích dẫn |
| `rag_chain` (Q5) | Unit | `test_format_context_includes_source_info` | `format_context()` hiển thị tên file và trang |
| `rag_chain` (Q6) | Unit | `test_chat_history_included_in_prompt` | `chat_history` được đưa vào prompt |
| `rag_chain` (Q6) | Unit | `test_reformulate_template_has_required_vars` | Template có `{chat_history}` và `{question}` |
| `rag_chain` (Q6) | Unit | `test_empty_history_no_reformulation` | Không có history → bỏ qua reformulation |
| `rag_chain` (Q6) | Unit | `test_ask_question_accepts_chat_history_param` | Hàm nhận tham số `chat_history` |
| `rag_chain` | Unit | `test_empty_list_returns_string` | `format_context([])` → chuỗi không rỗng |
| `rag_chain` | Unit | `test_single_document_contains_content` | Nội dung doc có trong output |
| `rag_chain` | Unit | `test_missing_metadata_no_crash` | Doc thiếu metadata → không crash |
| `rag_chain` | Unit | `test_returns_bool` | `check_ollama_connection()` → `bool` |
| `rag_chain` | Integration | `test_ask_question_no_vectorstore` | `ask_question()` không có VS → dict hợp lệ |
| `rag_chain` | Integration | `test_ask_question_empty_returns_error` | Câu hỏi rỗng → trả về `error` thân thiện |
| `rag_chain` | Integration | `test_ask_question_vietnamese` | Câu tiếng Việt → `language == 'vi'` |
| `vector_store` (Q8) | Unit | `test_similarity_search_with_file_filter` | filter=file1 → chỉ chunks từ file1 |
| `vector_store` (Q8) | Unit | `test_filter_excludes_wrong_source` | filter=file1 không trả về doc từ file2 |
| `vector_store` (Q8) | Unit | `test_filter_none_returns_all` | filter=None → tất cả kết quả |
| `vector_store` (Q8) | Unit | `test_filter_nonexistent_file_returns_empty` | filter=file_không_tồn_tại → `[]` |
| `vector_store` (Q8) | Unit | `test_filter_by_source_function` | `filter_by_source()` lọc đúng |
| `vector_store` (Q8) | Unit | `test_filter_preserves_metadata` | Metadata nguyên vẹn sau lọc |
| `vector_store` (Q8) | Unit | `test_filter_case_sensitive` | Filter phân biệt hoa thường |
| `vector_store` (Q8) | Integration | `test_create_and_search_with_filter` | Tạo VS thật → search có filter |
| `vector_store` (Q8) | Integration | `test_multiple_files_filter_isolation` | 2 file → filter riêng biệt |
| `vector_store` | Unit | `test_empty_documents_returns_none` | `create_vector_store([])` → `None` |
| `vector_store` | Unit | `test_exception_returns_empty_list` | VS lỗi → `similarity_search()` → `[]` |
| `vector_store` | Unit | `test_load_returns_none_when_no_index` | Không có index → `load_vector_store()` → `None` |
| `vector_store` | Integration | `test_create_and_search` | Tạo VS thật → search → kết quả Document |
| `vector_store` | Integration | `test_save_and_load_vector_store` | Lưu → tải lại → search vẫn hoạt động |
| `vector_store` | Integration | `test_singleton_embedding_model` | Gọi 2 lần → cùng object (singleton) |
| `hybrid_search` (Q7) | Unit | `test_ensemble_retriever_created` | `create_ensemble_retriever()` trả về `EnsembleRetriever` |
| `hybrid_search` (Q7) | Unit | `test_retriever_weights_correct` | weights = [0.6, 0.4] |
| `hybrid_search` (Q7) | Unit | `test_bm25_retriever_created` | `create_bm25_retriever()` trả về `BM25Retriever` |
| `hybrid_search` (Q7) | Unit | `test_hybrid_returns_documents` | Invoke → trả về danh sách Document |
| `hybrid_search` (Q7) | Integration | `test_hybrid_search_end_to_end` | FAISS + BM25 → kết quả lai |
| `reranker` (Q9) | Unit | `test_rerank_returns_sorted_list` | Kết quả sắp xếp theo cross score |
| `reranker` (Q9) | Unit | `test_rerank_top_k_respected` | Số kết quả ≤ top_k |
| `reranker` (Q9) | Unit | `test_scores_normalized_sigmoid` | Cross score ∈ [0, 1] |
| `reranker` (Q9) | Unit | `test_fallback_when_model_unavailable` | Model lỗi → trả về bi-encoder scores |
| `reranker` (Q9) | Integration | `test_cross_encoder_rerank_real` | Cross-encoder thật → rerank đúng thứ tự |
| `self_rag` (Q10) | Unit | `test_rewrite_query_returns_list` | `rewrite_query()` → list ≥ 1 câu hỏi |
| `self_rag` (Q10) | Unit | `test_grade_relevance_returns_co_or_khong` | Trả về `"CÓ"` hoặc `"KHÔNG"` |
| `self_rag` (Q10) | Unit | `test_grade_answer_returns_json` | `grade_answer()` → dict có `score`, `is_grounded`, `has_hallucination` |
| `self_rag` (Q10) | Unit | `test_grade_answer_score_in_range` | `score` ∈ [0.0, 1.0] |
| `self_rag` (Q10) | Integration | `test_rewrite_query_ollama` | Ollama thật → sinh 3 phiên bản câu hỏi |
| `self_rag` (Q10) | Integration | `test_grade_document_relevance_real` | Ollama thật → "CÓ"/"KHÔNG" hợp lệ |
| `self_rag` (Q10) | Integration | `test_grade_answer_real` | Ollama thật → JSON hợp lệ |

### **4.3.3. Chiến Lược Kiểm Thử** {#433-chiến-lược-kiểm-thử}
...
- **Test Coverage:** Bộ kiểm thử bao phủ toàn bộ 10 yêu cầu kỹ thuật (Q1-Q10): xử lý tài liệu đa định dạng (Q1), giao diện hội thoại (Q2, Q3), chunking (Q4), citation tracking (Q5), conversational RAG (Q6), hybrid search (Q7), metadata filtering (Q8), re-ranking (Q9), và Self-RAG (Q10).

### **4.3.4. Lệnh Chạy Kiểm Thử** {#434-lệnh-chạy-kiểm-thử}
...
## **4.4. Trực Quan Hóa Dữ Liệu** {#44-trực-quan-hóa-dữ-liệu}  
![][image2]  
Biểu đồ Thời gian truy xuất theo kích thước dữ liệu (Latency vs Data Size)Biểu đồ Thời gian truy xuất theo kích thước dữ liệu (Latency vs Data Size)  
**Hình 1\.** Biểu đồ so sánh thời gian truy xuất (ms) của 3 phương pháp ứng với các kích cỡ Vector Store khác nhau.  
Biểu đồ đường cho thấy một điểm đáng chú ý: Thời gian truy xuất của FAISS gần như đi ngang khi số lượng chunk tăng từ 1.000 lên 10.000 (dao động trong khoảng 40–50ms). Đây là đặc tính mong muốn của FAISS — thuật toán tìm kiếm vector SIMD/AVX được tối ưu phần cứng, nên phần search chỉ tốn 1–8ms và bị che lấp bởi chi phí encode câu truy vấn cố định (~40ms).

BM25 tăng tuyến tính rõ ràng (3.15ms → 35.77ms) vì không có hardware acceleration tương đương. Hybrid Search tăng vừa phải (42.54ms → 74.65ms), phản ánh tổng hợp của cả hai phương pháp.

*Khuyến nghị:*

* **Hybrid Search** (mặc định): chất lượng context tốt nhất ở mọi quy mô (~43–75ms), kết hợp ưu điểm tìm kiếm ngữ nghĩa của FAISS và tìm kiếm từ khóa của BM25.
* **FAISS thuần**: khi cần tốc độ tuyệt đối, độ trễ ổn định ~40ms.
* **Lưu ý**: Khi kho tài liệu tăng lên hàng chục nghìn chunks, BM25 có thể trở thành điểm nghẽn — cần cân nhắc giảm trọng số BM25.
# **CHƯƠNG V: GIAO DIỆN VÀ HƯỚNG DẪN SỬ DỤNG** {#chương-v-giao-diện-và-hướng-dẫn-sử-dụng}
...
## **5.3. Các Tính Năng Chính** {#53-các-tính-năng-chính}
...
Chương này trình bày chi tiết 10 yêu cầu kỹ thuật (Q1-Q10) của đề tài, giải thích giải pháp triển khai, code minh họa từ source thực tế, và bằng chứng kiểm thử.

## **6.1. Q1 — Xử Lý Tài Liệu Đa Định Dạng (PDF & DOCX)** {#61-q1--xử-lý-tài-liệu-đa-định-dạng-pdf--docx}

**Yêu cầu:** Hệ thống phải đọc và xử lý được cả file PDF và DOCX, trích xuất văn bản kèm metadata (tên file, số trang).

**Giải pháp triển khai** trong `modules/document_processor.py`:

```python
SUPPORTED_EXTENSIONS = {".pdf", ".docx"}

def extract_text_from_pdf(file_path: str, source_name: str) -> List[Document]:
    """Trích xuất văn bản từ PDF, trả về List[Document] theo từng trang."""
    documents = []
...
def extract_text_from_docx(file_path: str, source_name: str) -> List[Document]:
    """Trích xuất văn bản từ DOCX, gộp paragraphs thành 'trang ảo'."""
    from docx import Document as DocxDocument
    doc = DocxDocument(file_path)
    paragraphs = [p.text for p in doc.paragraphs if p.text.strip()]
    # Ghép mỗi 20 đoạn thành một "trang ảo"
    ...
```

**Kết quả kiểm thử:**
```
tests/test_document_processor.py::test_extract_pdf PASSED
tests/test_document_processor.py::test_extract_docx PASSED
tests/test_document_processor.py::test_unsupported_extension_raises PASSED
```

## **6.2. Q2 — Lưu Trữ Lịch Sử Hội Thoại** {#62-q2--lưu-trữ-lịch-sử-hội-thoại}

**Yêu cầu:** Hệ thống phải lưu lịch sử hội thoại trong phiên làm việc và sử dụng lịch sử đó cho các câu hỏi tiếp theo.

**Giải pháp triển khai** trong `app.py`:
...
# Thêm vào lịch sử sau mỗi lượt hội thoại
st.session_state.chat_history.append(HumanMessage(content=user_question))
st.session_state.chat_history.append(AIMessage(content=answer))

# Truyền vào pipeline RAG
result = ask_question(
...
**Yêu cầu:** Người dùng phải có thể xóa lịch sử hội thoại và/hoặc xóa toàn bộ Vector Store để bắt đầu phiên mới.

**Giải pháp triển khai** trong sidebar của `app.py`:
...
# Nút xóa Vector Store
if st.sidebar.button("🗑️ Xóa Vector Store"):
    st.session_state.vector_store = None
    st.session_state.chat_history = []
    # Xóa files đã upload và index FAISS trên đĩa
    import shutil
...
Hai nút này hoàn toàn độc lập: "Xóa lịch sử" chỉ reset conversation trong khi vẫn giữ nguyên tài liệu đã index; "Xóa Vector Store" xóa cả index FAISS và lịch sử.

## **6.4. Q4 — Chunking Tài Liệu (Phân Đoạn Văn Bản)** {#64-q4--chunking-tài-liệu-phân-đoạn-văn-bản}

**Yêu cầu:** Hệ thống phải chia nhỏ tài liệu thành các đoạn (chunks) có kích thước phù hợp để embed và retrieve hiệu quả.
...
def split_documents(documents: List[Document]) -> List[Document]:
    """Chia nhỏ tài liệu thành chunks với overlap để bảo toàn ngữ cảnh."""
    text_splitter = RecursiveCharacterTextSplitter(
        chunk_size=config.CHUNK_SIZE,       # 1500 ký tự
        chunk_overlap=config.CHUNK_OVERLAP, # 200 ký tự overlap
        length_function=len,
        separators=["\n\n", "\n", ".", "!", "?", ",", " ", ""]
    )
...
**Lý do chọn các tham số (`config.py`):**

| Tham số | Giá trị | Lý do |
|---|---|---|
| `CHUNK_SIZE = 1500` | 1500 ký tự | ~300-400 tokens — phù hợp với context window và đủ ngữ cảnh cho câu trả lời |
| `CHUNK_OVERLAP = 200` | 200 ký tự | ~13% overlap — đảm bảo câu quan trọng ở ranh giới chunk không bị mất |

`RecursiveCharacterTextSplitter` ưu tiên tách tại `\n\n` (đoạn văn), sau đó `\n` (dòng), sau đó dấu câu, đảm bảo chunks không cắt giữa câu.

## **6.5. Q5 — Citation Tracking (Truy Dấu Nguồn)** {#65-q5--citation-tracking-truy-dấu-nguồn}

**Yêu cầu:** Câu trả lời phải kèm thông tin trích dẫn nguồn (tên file, số trang) để người dùng kiểm chứng.

**Giải pháp triển khai** trong `modules/rag_chain.py`:

```python
def ask_question(question: str, vector_store, chat_history=None,
                 file_filter=None) -> dict:
    """Thực hiện RAG pipeline và trả về answer kèm sources."""
    # ... retrieval + generation ...
    
    # Thu thập sources từ metadata của retrieved documents
    sources = []
    seen = set()
    ...
    return {
        "answer": answer_text,
        "sources": sources,   # List[{"file": str, "page": int}]
...
if result["sources"]:
    ...
    for src in result["sources"]:
        st.markdown(f"- 📄 `{src['file']}` — Trang {src['page']}")
```

**Kết quả kiểm thử (Q5 — TestCitationSourceTracking, 4 tests passed):**
```
test_sources_returned_in_response PASSED
test_source_contains_file_and_page PASSED
test_sources_deduplicated PASSED
test_format_context_includes_source_info PASSED
```

## **6.6. Q6 — Conversational RAG (Hội Thoại Đa Lượt)** {#66-q6--conversational-rag-hội-thoại-đa-lượt}

**Yêu cầu:** Hệ thống phải hiểu và xử lý câu hỏi trong ngữ cảnh hội thoại liên tục — khi người dùng dùng đại từ tham chiếu ("nó", "điều đó") hoặc hỏi tiếp theo mà không nhắc lại chủ đề.

**Giải pháp triển khai** — Question Reformulation trong `modules/rag_chain.py`:

```python
REFORMULATE_QUESTION_TEMPLATE = """Dựa vào lịch sử hội thoại bên dưới và câu hỏi tiếp theo,
hãy viết lại câu hỏi thành một câu hoàn chỉnh, độc lập (standalone question).
...
def ask_question(question, vector_store, chat_history=None, file_filter=None):
    # Nếu có lịch sử, reformulate câu hỏi trước khi retrieve
    ...
    # Tiếp tục retrieve với standalone_question
    docs = retriever.invoke(standalone_question)
    ...
```

**Kết quả kiểm thử (Q6 — TestConversationalRAG, 4 tests passed):**
```
test_chat_history_included_in_prompt PASSED
test_reformulate_template_has_required_vars PASSED
test_empty_history_no_reformulation PASSED
test_ask_question_accepts_chat_history_param PASSED
```

## **6.7. Q7 — Hybrid Search (FAISS + BM25)** {#67-q7--hybrid-search-faiss--bm25}

**Yêu cầu:** Kết hợp semantic search (FAISS) và keyword search (BM25) để tăng độ phủ và chính xác trong việc tìm tài liệu liên quan.

**Giải pháp triển khai** trong `modules/vector_store.py`:
...
def create_bm25_retriever(documents: List[Document]) -> BM25Retriever:
    """Tạo BM25 retriever từ danh sách documents."""
    retriever = BM25Retriever.from_documents(documents)
    retriever.k = config.HYBRID_TOP_K  # Top-5
    return retriever

def create_ensemble_retriever(vector_store, bm25_retriever) -> EnsembleRetriever:
    """Kết hợp FAISS và BM25 với Reciprocal Rank Fusion."""
    faiss_retriever = vector_store.as_retriever(
        search_type=config.RETRIEVAL_SEARCH_TYPE,  # "mmr"
        search_kwargs={"k": config.HYBRID_TOP_K}
    )
    return EnsembleRetriever(
        retrievers=[faiss_retriever, bm25_retriever],
        weights=[config.HYBRID_VECTOR_WEIGHT,   # 0.6 — FAISS
                 config.HYBRID_BM25_WEIGHT]     # 0.4 — BM25
    )
```

**Kết quả kiểm thử (Q7, 5 tests passed):**
```
test_ensemble_retriever_created PASSED
test_retriever_weights_correct PASSED
test_bm25_retriever_created PASSED
test_hybrid_returns_documents PASSED
test_hybrid_search_end_to_end PASSED (integration)
```

## **6.8. Q8 — Metadata Filtering (Lọc Theo Tài Liệu Nguồn)** {#68-q8--metadata-filtering-lọc-theo-tài-liệu-nguồn}

**Yêu cầu:** Người dùng có thể giới hạn tìm kiếm chỉ trong một file cụ thể, loại bỏ noise từ các tài liệu không liên quan.

**Giải pháp triển khai** trong `modules/vector_store.py`:

```python
def similarity_search(vector_store, query: str, top_k: int = None,
                      file_filter: str = None) -> List[Document]:
    """Tìm kiếm với tùy chọn lọc theo tên file nguồn."""
    k = top_k or config.RETRIEVAL_TOP_K
    
    if file_filter:
        # Lọc theo metadata "source" trước khi trả kết quả
        all_docs = vector_store.similarity_search(query, k=k * 3)
        filtered = [
            doc for doc in all_docs
            if doc.metadata.get("source") == file_filter
        ]
        return filtered[:k]
    else:
        return vector_store.similarity_search(query, k=k)

def filter_by_source(documents: List[Document],
                     file_filter: str) -> List[Document]:
    """Hàm tiện ích lọc danh sách documents theo source metadata."""
    return [
...
**Kết quả kiểm thử (Q8 — TestMetadataFiltering, 9 tests passed):**
```
test_similarity_search_with_file_filter PASSED
test_filter_excludes_wrong_source PASSED
test_filter_none_returns_all PASSED
test_filter_nonexistent_file_returns_empty PASSED
test_filter_by_source_function PASSED
test_filter_preserves_metadata PASSED
test_filter_case_sensitive PASSED
test_create_and_search_with_filter PASSED (integration)
test_multiple_files_filter_isolation PASSED (integration)
```

## **6.9. Q9 — Cross-Encoder Re-Ranking** {#69-q9--cross-encoder-re-ranking}

**Yêu cầu:** Áp dụng Cross-Encoder để xếp hạng lại kết quả retrieve, cải thiện chất lượng context đưa vào LLM.

**Giải pháp triển khai** trong `modules/reranker.py`:
...
def get_cross_encoder() -> CrossEncoder:
    """Singleton — tải model Cross-Encoder một lần."""
...
def rerank_with_cross_encoder(
    ...
    """
    Xếp hạng lại documents sử dụng Cross-Encoder.
    Returns: List[(Document, bi_encoder_score, cross_encoder_score)]
    """
    try:
        ...
        # Chuẩn hóa sigmoid về [0, 1]
        normalized = [1 / (1 + np.exp(-s)) for s in raw_scores]
        
        # Kết hợp với bi-encoder score và sắp xếp
        results = [
            (doc, bi_score, cross_score)
            for (doc, bi_score), cross_score
            in zip(doc_score_pairs, normalized)
        ]
        results.sort(key=lambda x: x[2], reverse=True)
        return results[:top_k]
    except Exception:
        # Fallback: trả về bi-encoder scores nếu cross-encoder lỗi
        return [(doc, score, score) for doc, score in doc_score_pairs[:top_k]]
```

**Kết quả kiểm thử (Q9, 5 tests passed):**
```
test_rerank_returns_sorted_list PASSED
test_rerank_top_k_respected PASSED
test_scores_normalized_sigmoid PASSED
test_fallback_when_model_unavailable PASSED
test_cross_encoder_rerank_real PASSED (integration)
```

## **6.10. Q10 — Self-RAG (Tự Đánh Giá và Cải Thiện)** {#610-q10--self-rag-tự-đánh-giá-và-cải-thiện}

**Yêu cầu:** Hệ thống tự đánh giá chất lượng retrieval và generation, phát hiện hallucination.

**Giải pháp triển khai** trong `modules/self_rag.py`:

```python
# Template 1: Query Rewriting
QUERY_REWRITE_TEMPLATE = """Bạn là chuyên gia tối ưu hóa câu truy vấn.
Hãy viết lại câu hỏi sau thành 3 phiên bản khác nhau...
Câu hỏi gốc: {question}
Trả về dạng danh sách đánh số."""

# Template 2: Document Relevance Grading
RELEVANCE_CHECK_TEMPLATE = """Đánh giá xem tài liệu có liên quan đến câu hỏi không.
Câu hỏi: {question}
Tài liệu: {context}
Trả lời CHỈ "CÓ" hoặc "KHÔNG"."""

# Template 3: Answer Grading
ANSWER_GRADING_TEMPLATE = """Đánh giá câu trả lời theo các tiêu chí sau.
Trả về JSON: {{"score": 0.0-1.0, "is_grounded": bool,
              "has_hallucination": bool, "feedback": "..."}}
Câu hỏi: {question}
Context: {context}
Câu trả lời: {answer}"""

def rewrite_query(question: str, llm=None) -> List[str]:
    """Sinh 3 phiên bản câu hỏi để mở rộng không gian tìm kiếm."""
    chain = rewrite_prompt | get_llm(llm) | StrOutputParser()
    result = chain.invoke({"question": question})
    # Parse numbered list từ output
    lines = [l.strip() for l in result.split("\n") if l.strip()]
    queries = [l.lstrip("0123456789.-) ") for l in lines if l[0].isdigit()]
    return queries if queries else [question]

def grade_document_relevance(question: str, document: str,
                              llm=None) -> str:
    """Trả về 'CÓ' nếu document liên quan, 'KHÔNG' nếu không."""
    chain = relevance_prompt | get_llm(llm) | StrOutputParser()
    result = chain.invoke({"question": question, "context": document})
    return "CÓ" if "CÓ" in result.upper() else "KHÔNG"

def grade_answer(question: str, context: str, answer: str,
                  llm=None) -> dict:
    """
    Đánh giá chất lượng câu trả lời.
    Returns: {"score": float, "is_grounded": bool,
              "has_hallucination": bool, "feedback": str}
    """
    chain = grading_prompt | get_llm(llm) | StrOutputParser()
    result = chain.invoke({
...
**Kết quả kiểm thử (Q10, 7 tests passed):**
```
test_rewrite_query_returns_list PASSED
test_grade_relevance_returns_co_or_khong PASSED
test_grade_answer_returns_json PASSED
test_grade_answer_score_in_range PASSED
test_rewrite_query_ollama PASSED (integration)
test_grade_document_relevance_real PASSED (integration)
test_grade_answer_real PASSED (integration)
```
...
Về mặt kỹ thuật, hệ thống đã tích hợp trơn tru mô hình ngôn ngữ lớn (Qwen2.5:7b thông qua Ollama) kết hợp với framework LangChain và giao diện trực quan Streamlit. Hệ thống triển khai đồng thời ba kỹ thuật nâng cao: **Hybrid Search** (kết hợp FAISS vector search và BM25 keyword search với trọng số 0.6/0.4) để tìm kiếm toàn diện hơn, **Cross-encoder Reranking** (`cross-encoder/ms-marco-MiniLM-L-6-v2`) để sắp xếp lại kết quả theo độ liên quan thực sự, và **Self-RAG** để tự đánh giá chất lượng tài liệu và câu trả lời trước khi trả về người dùng.
